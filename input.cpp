...
#define MAPPING_TYPE_BASE      0
#define MAPPING_TYPE_ALT       1
...
void start_map_setting(int cnt, int set)
{
	...
	{
		mapping_dev = -1;
		mapping_type = (cnt < 0) ? MAPPING_TYPE_JOYKEYMAP : cnt ? MAPPING_TYPE_ALT : MAPPING_TYPE_KBDMAP;
	}
	mapping_count = cnt;
	mapping_clear = 0;
	tmp_axis_n = 0;

	if (mapping_type <= MAPPING_TYPE_ALT && is_menu_core()) mapping_button = -6; // for axis
...
static void input_cb(struct input_event *ev, struct input_absinfo *absinfo, int dev)
{
	...
	int map_skip = (ev->type == EV_KEY && ev->code == KEY_SPACE && ((mapping_dev >= 0 && mapping_type==MAPPING_TYPE_ALT) || mapping_button<0));
	...
	//mapping
	if (mapping && (mapping_dev >= 0 || ev->value) && !((mapping_type <= MAPPING_TYPE_ALT || !mapping_button) && (cancel || enter)))
	{
		if (is_menu_core())
		{
		...
		if (ev->type == EV_KEY && mapping_button>=0 && !osd_event)
		{
			...
			else
			{
				...
				{
					mapping_dev = dev;
					mapping_type = (ev->code >= 256) ? MAPPING_TYPE_ALT : MAPPING_TYPE_BASE;
					key_mapped = 0;
				}
					...
					else if (ev->type == EV_KEY && ev->value == 1)
					{
						...
						if (!map_skip)
						{
							...
							if (ev->code < 256)
							{
								...
								mapping_button += 4;
								mapping_type = MAPPING_TYPE_BASE;
							}
						}
					}
				}
		...
		if (mapping_type <= MAPPING_TYPE_ALT && map_skip && mapping_button < mapping_count)
		{
			if (ev->value == 1)
			{
				if (mapping_dev >= 0)
		...
		if (is_menu_core() && mapping_type <= MAPPING_TYPE_ALT && mapping_dev >= 0)
		{
					...
					if (!input[i].mouse)
					{
						...
						if (read(pool[i].fd, &ev, sizeof(ev)) == sizeof(ev))
						{
							...
							else if (ev.type)
							{
								...
								if (ev.type == EV_ABS && !(mapping && mapping_type <= MAPPING_TYPE_ALT && mapping_button < -4) && !(ev.code <= 1 && input[dev].lightgun))
								{
									input_absinfo *pai = 0;
									uint8_t axis_edge = 0;
...
void set_joy_backspace(uint32_t button_idx) 
{
	if(button_idx > 16) button_idx=16;
	uint16_t mask = 0b1 << button_idx;
	//printf("OSD Backspace set to: %s\n", std::bitset<16>(mask).to_string().c_str());
	osd_backspace = mask;	
}
